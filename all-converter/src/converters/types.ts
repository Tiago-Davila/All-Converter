...
export interface Converter { id: string; label: string; from: readonly ConverterSource[]; to: string; maxSizeMB: number; limitation?: string; targetsFor?(type: DetectedFileType): readonly string[]; convert(file: File, onProgress: (progress: ConversionProgress) => void, options: Record<string, unknown>, signal: AbortSignal): Promise<ConversionResult[]>; convertMany?(files: readonly File[], onProgress: (progress: ConversionProgress) => void, options: Record<string, unknown>, signal: AbortSignal): Promise<ConversionResult[]> }
