...
export interface Converter { id: string; label: string; from: FileKind[]; to: string; maxSizeMB: number; convert(file: File, onProgress: (progress: ConversionProgress) => void, options: Record<string, unknown>, signal: AbortSignal): Promise<ConversionResult[]> }
