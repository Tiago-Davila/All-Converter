...
interface DropzoneProps { onFiles(files: IncomingFile[], skipped?: number): void }
...
export function Dropzone({ onFiles }: DropzoneProps) {
  ...
  return <section aria-label="Agregar archivos" onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); if (event.dataTransfer.items.length) void readDroppedItems(event.dataTransfer.items).then(({ files, skipped }) => onFiles(files, skipped)); else accept(event.dataTransfer.files) }}>
