...
- [ ] T036 [US4] Pruebas del `SoundManager` cubriendo los **10 invariantes** de `contracts/sound.md` en `tests/ui/sound/SoundManager.test.ts`: no suena sin preferencia · no suena bajo reduce-motion **aunque esté habilitado** · cola de 10 → **1** sonido · soltar 10 archivos → **1** `drop` · segundo disparo mientras suena → descartado · sin Web Audio → no-op sin throw · **cola cancelada → sin sonido de éxito** · los 4 `silenceReason` (depende de T035).
- [ ] T037 [US4] Cablear los 4 eventos a sus disparadores reales y **verificar el equivalente visual de cada uno** (drop → filas aparecen; reject → tile con motivo; fin de cola → filas `done`/`error` + ZipBar) en `src/App.tsx` y `tests/ui/sound/wiring.test.tsx`. Cubre FR-029, FR-033 (depende de T035, T025).
